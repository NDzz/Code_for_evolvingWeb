...
//////FUNCTIONS
//main functions for animation and markings of the canvas
function animationLoop(){
	clear();

	//data are classified into functions
	scoreDisplay();
	markings();
	drawSprite(goalie);
	drawAllBall();
...
//function for the marking of the soccer field
function markings(){
	...
	lines(); //middle line
	midCircle();//big circle in the middle

	endSquare(goal);// square shape in goalpost to trigger gameover
}
...
//function dimension for the balls
...
////////functions to draw the pitch (markings)
function lines(){
	ctx.beginPath();
	ctx.moveTo(0,h/2);
	ctx.lineTo(w,h/2);
...
function setUpCanvas(){
	...
	canvas.style.border = "2px solid black";
		canvas.style.background = "#30B52E";
